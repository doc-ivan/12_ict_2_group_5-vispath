...
import { getFirestore, 
        ...
        startAfter,
        Timestamp,
        deleteDoc } from "https://www.gstatic.com/firebasejs/12.0.0/firebase-firestore.js";
...
let indicators = document.querySelectorAll('.carousel-indicator span')
...
// Store stories globally for indicator navigation
let carouselStories = []
...
const yourStoryContainer = document.getElementById("yourStorySection")
...
storyInput.addEventListener('input', function () {
    ...
    charCount.textContent = `${charCountValue} character${charCountValue !== 1 && charCountValue !== 0 ? 's' : ''}`

    // Enable/disable share button
    shareBtn.disabled = charCountValue === 0
})
...
shareBtn.addEventListener('click', async function() {
    ...
    if(userDocSnap.exists() && userDocSnap.data().role === "user") { 
        ...
        if (storyInput.value.trim()) {
            ...
            setTimeout(() => {
                storyInput.value = ''
                charCount.textContent = '0 character'
...
async function loadHeaderCarousel() {
    ...
    // Store stories for indicator clicks
    carouselStories = stories
    ...
    startDelayTimer = setTimeout(() => {
        ...
        // TIMER – dito nagpapalit ang story
        carouselTimer = setInterval(() => {
            currentIndexForCarouselStory = (currentIndexForCarouselStory + 1) %
            stories.length
            renderHeaderSlide(stories[currentIndexForCarouselStory])
            changeIndicator()
        }, DISPLAY_TIME)
...
function changeIndicator() {
    indicators.forEach(ind => {
        ...
        console.log("change indicator function")
    })
    indicators[currentIndexForCarouselStory].classList.add('active')
    
}

// NEW FEATURE: Setup click handlers for carousel indicators
function setupIndicatorClickHandlers() {
    ...
    // Re-query indicators after cloning
    indicators = document.querySelectorAll('.carousel-indicator span')
    indicators.forEach((indicator, index) => {
        indicator.addEventListener('click', () => {
            // Only navigate if clicking a different indicator
            if (index !== currentIndexForCarouselStory && carouselStories[index]) {

                // Update current index
                currentIndexForCarouselStory = index

                // Render the selected story
                renderHeaderSlide(carouselStories[currentIndexForCarouselStory])
        ...
        onAuthStateChanged(auth, async (user) => {
            ...
            const yourStories = userData.yourStory || []

            if (yourStories.length === 0) {
                console.log("No uploaded stories yet")
                return
            }


            yourStoryContainer.innerHTML = ""

            const sectionTitle = document.createElement("h3")
            sectionTitle.className = "section-title"
            sectionTitle.innerHTML = "Your Uploaded Story"

            yourStoryContainer.appendChild(sectionTitle)


            // Fetch each story document
            for (const storyId of yourStories) {

                const storyRef = doc(db, 'stories', storyId)
                const storySnap = await getDoc(storyRef)

                if (storySnap.exists()) {
                    renderYourStory(storySnap)
                }
        ...
        function renderYourStory(storySnap) {
            const storyData = storySnap.data()

            const storyCard = document.createElement("div")
            storyCard.className = "story-card"

            const storytThumbnail = document.createElement("div")
            storytThumbnail.className = "story-thumbnail"

            const storyDetails = document.createElement("div")
            storyDetails.className = "story-details"

            const deleteBtnYourStory = document.createElement("button")
            deleteBtnYourStory.className = "delete-btn-your-story"


            storytThumbnail.innerHTML = `
                <img src="${storyData.coverUrl}" alt="Story Cover">
            `

            storyDetails.innerHTML = `
                <h3 class="story-title">${toTitleCase(storyData.title)}</h3>
                <p class="story-year">${storyData.uploadedAt.toDate().getFullYear()}</p>
                <span class="story-status">✓ Published</span>
            `

            deleteBtnYourStory.textContent = "Delete"


            storyCard.appendChild(storytThumbnail)
            storyCard.appendChild(storyDetails)
            storyCard.appendChild(deleteBtnYourStory)
            yourStoryContainer.appendChild(storyCard)

            storytThumbnail.addEventListener("click", () => {
                changeHashToReading(storySnap.id)
            })

            storyDetails.addEventListener("click", () => {
                changeHashToReading(storySnap.id)
            })

            deleteBtnYourStory.addEventListener("click", () => {
                showDeleteModal(storySnap.id)
            })
        ...
        function confirmDelete(storyId) {
            ...
            deleteStoryDoc(storyId)
            
            setTimeout(() => {
                closeDeleteModal()
                alert('Story deleted successfully.\n\nYour story has been permanently removed from the website.')
            }, 300)
        }
        ...
        function handleFile(file) {
            ...
            if (!validTypes.includes(fileExtension) && !validTypes.includes(file.type)) {
                showError('Invalid file type. Please upload only PDF or DOCX files.')
                return
            }
        ...
        async function uploadFile(file, user) {
            ...
            return data.path // Returns the path of the uploaded file
        }
        ...
        async function addToQueue(url, userRef, file) {
            const newStory = {
                url: url,
                name: file.name,
                status: "waiting",
            ...
            await updateDoc(userRef, {
                inQueueStory: arrayUnion(newStory)
            })

            removeFile()
            alert("File uploaded and saved to your queue!")
        }
        ...
        async function handleUpload(file) {
            ...
            if (!user) {
                ...
                if(userSnap.exists() && userSnap.data().role === "user") { 
                    ...
                    if (!data.inQueueStory) {
                        await updateDoc(userRef, { inQueueStory: [] })
                        data.inQueueStory = []
                    }

                    if (data.inQueueStory.length >= 2) {
                        showError("You already have 2 stories in queue.")
                        return
                    }

                    // Step 2: Upload to Supabase
                    const path = await uploadFile(file, user)
                    if (!path) return

                    // Step 3: Get public URL
                    const publicUrl = getPublicUrl(path)

                    // Step 4: Save to Firestore array
                    await addToQueue(publicUrl, userRef, file)

                } else {
                    showError("You can't use admin account to upload.")
                    removeFile()
                    return // Stop function
                }
            }
        ...
        <div class="synopsis-header-wrapper">
            ...
            <div class="synopsis-top-header-container">
                ...
                <div class="synopsis-top-header-container-details">
                    <h1>${toTitleCase(story.title)}</h1>
                    <p><i class="fa-regular fa-user"></i> ${toTitleCase(story.author)}</p>
                    <div>
                        <p><i class="fa-solid fa-eye"></i> Views: ${story.views}</p>
                        <p><i class="fa-solid fa-book"></i></i> Type: ${toTitleCase(story.type)}</p>
                        <p><i class="fa-solid fa-language"></i> Language: ${toTitleCase(story.language)}</p>
                        <p><i class="fa-solid fa-calendar-days"></i> Uploaded: ${story.uploadedAt.toDate().getFullYear()}</p>
                    </div>
                    <button data-id= "${docSnap.id}"><i class="fa-solid fa-book-open"></i> Read Now</button>
                </div>
            </div>
        </div>
...
// Load saved theme
if (localStorage.getItem("readingTheme") === "dark") {
    readingPlaceContainer.classList.add("dark-mode")
...
async function renderReadingStory(docSnap, docSnapStoryInfo) {
    ...
    const themeBtn = headerEl.querySelector("#theme-change")
    ...
    themeBtn.addEventListener("click", async () => {
        ...
        readingPlaceContainer.classList.toggle("dark-mode")

        if (readingPlaceContainer.classList.contains("dark-mode")) {
...
function homeSectionRenderContinueStoriesFromDB(doc) {
    ...
    storyEl.innerHTML = `
        ...
        <div class="year-type-container">
            <p><i class="fa-solid fa-calendar-days"></i> ${story.uploadedAt.toDate().getFullYear()}</p>
            <p><i class="fa-solid fa-book"></i> ${toTitleCase(story.type)}</p>
        </div>
    `
